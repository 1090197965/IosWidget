...
async function collect() {
  ...
  data.target = widgetConfig.target || '';
...
async function getLocationImg(data: IRecordData) {
  ...
  log(url);
  return await getImageByUrl(url, `temp_v3_${x}_${y}`);
}
...
async function createWidget(data: IRecordData) {
  try {
    ...
    const batteryLevel = Math.floor(data.batteryLevel * 1000) / 10;
    const batteryLevelFloat = batteryLevel / 100;
    const batteryIsCharging = data.isCharging;
    ...
    const titleBgStack = floor.addStack();
    const infoWidth = Math.floor(width / 2);
    const infoHeight = 45;
    titleBgStack.centerAlignContent();
    titleBgStack.size = new Size(infoWidth, infoHeight);
    titleBgStack.backgroundColor = new Color('#000000', 0.3);
    titleBgStack.cornerRadius = 12;

    titleBgStack.addSpacer(10);

    // 电池电量百分比
    const titleStack = titleBgStack.addStack();
    const titleText = titleStack.addText(`${batteryLevel}% 1`);
    titleText.font = Font.systemFont(14);
    titleStack.addSpacer(5);

    // 电池本体功能
    const battery = titleBgStack.addStack();
    ...
    const remainWidth = parseInt(
      (
        (batteryLevel < 0.1 ? 0.1 : batteryLevel) *
        (size.width - border * 4)
      ).toFixed(),
    );
    ...
    space.backgroundColor = new Color('#666666', 0.8);
    space.cornerRadius = 2;
    space.setPadding(border, border, border, border);

    // 剩余电量
    const remain = space.addStack();
    // space.addSpacer(border);
    // const spaceLeft = space.addStack();
    // spaceLeft.size = new Size(border, size.height - border * 4);
    ...
    // 电器正极头
    const batteryHead = titleBgStack.addStack();
