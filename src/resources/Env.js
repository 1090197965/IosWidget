...
const createWidget = async ({
  ...
  texts['battery']
    ? battery(w, title)
    : provideText(title, w, textFormat.title);
  for (const text in texts) {
    if (
      text != 'battery' &&
      text != 'updateTime' &&
      texts.hasOwnProperty(text) &&
      texts[text]
    ) {
  ...
  texts['updateTime']
    ? provideText(`[更新] ${time('MM-dd HH:mm')}`, w, textFormat.SFMono)
    : ``;
...
const setupSunrise = async () => {
  ...
  const sunCacheDate = sunCacheExists
    ? files.modificationDate(sunCachePath)
    : 0;
  var sunDataRaw;
...
function provideBatteryIcon() {
  ...
  draw.drawImageInRect(
    SFSymbol.named('battery.0').image,
