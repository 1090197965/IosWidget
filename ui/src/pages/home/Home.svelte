<script>
    ...
    //将"lang="后面的所有文字替换成指定的文字
    const replaceLangFun = (str, lang) => {
        return str.includes('?') ? str.replace(/lang=[a-zA-Z_]+/, `lang=${lang}`) : str + '?lang=en_US';
    };
...
<div class="flex flex-col py-4">
    ...
    <div class="flex text-xs justify-around p-4 text-primary dark:text-dark underline">
        <div>
            <a href={replaceLangFun(window.location.href, isZh ? 'en_US' : 'zh_CN')} rel="noreferrer">
                {isZh ? 'English' : '简体中文'}
            </a>
        </div>
    </div>
