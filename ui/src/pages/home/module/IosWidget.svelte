...
<div class="flex flex-col py-1">
  {#if target && drive}
    <div class="flex-col bg-cover h-40 bg-no-repeat relative rounded-xl" style='{`background-image: url("${target.backgroundImg}")`}'>
      <div class="absolute bottom-0 left-0 bg-[#696969] opacity-60 rounded-xl w-32 h-11 m-2 pl-2" style="">
        <div class="text-white text-sm">↙{getTime(target.time)} ↗{getTime(drive.time)}</div>
        <div class="text-white text-sm">
          ...
          <div class="relative inline-block">
            ...
            <div class={`absolute opacity-70`} style={`
